...
parser.add_argument("run_dir", type=str,
                    help="Path to the run directory (local or Drive-mounted)")
parser.add_argument("--step", type=int, default=5,
                    help="Sample every N rounds (default: 5). Last round is always included.")
...
all_rounds = sorted([d for d in os.listdir(TRAIN_DIR) if d.startswith("round_")],
                    key=lambda x: int(x.split("_")[1]))

# sample every STEP rounds, always include the last one
sampled = all_rounds[::STEP]
if all_rounds[-1] not in sampled:
    sampled.append(all_rounds[-1])
print(f"Found {len(all_rounds)} rounds — sampling every {STEP} → {len(sampled)} rounds")
print(f"Sampled: {[r for r in sampled]}")

# ── Detect grid dimensions from roadnet ──────────────────────────────────────
first_gen = os.path.join(TRAIN_DIR, sampled[0],
                         sorted([d for d in os.listdir(os.path.join(TRAIN_DIR, sampled[0]))
...
for i, round_name in enumerate(sampled):
    round_idx = int(round_name.split("_")[1])
    print(f"  Loading round {round_idx} ({i+1}/{len(sampled)})...", flush=True)
    ...
    for idx in range(NUM_INTERSECTIONS):
        path = os.path.join(gen_dir, f"inter_{idx}.pkl")
    ...
    queue_by_round[round_idx] = (times_ref, np.array(queues_this_round))

    dfs = []
    for idx in range(NUM_INTERSECTIONS):
        path = os.path.join(gen_dir, f"vehicle_inter_{idx}.csv")
...
print(f"Loaded data for rounds: {sorted(queue_by_round.keys())}")
...
# ── PLOT 1: Mean queue over time per sampled round ───────────────────────────
...
ax.set_title(f"Mean queue over time — sampled rounds (every {STEP})")
...
# ── PLOT 5: Queue per intersection — small multiples (last round) ─────────────
_, qmat_last = queue_by_round[last_round]
...
# ── PLOT 6: Learning curve (sampled rounds) ───────────────────────────────────
round_stats = []
...
ax1.set_title(f"Learning curve (sampled every {STEP} rounds)")
...
# ── Turn movement congestion (last round) ─────────────────────────────────────
...
for row in range(NUM_ROW):
    ...
    for col in range(NUM_COL):
        ...
        for rec in records:
            stopped = rec["state"]["lane_num_vehicle_been_stopped_thres1"]
            if stopped:
                ew_queues.append(sum(stopped[0:3]) + sum(stopped[3:6]))
...
for col in range(NUM_COL):
    ...
    for row in range(NUM_ROW):
        ...
        for rec in records:
            stopped = rec["state"]["lane_num_vehicle_been_stopped_thres1"]
            if stopped:
                ns_queues.append(sum(stopped[6:9]) + sum(stopped[9:12]))
